...
function postprocess(source) {
  ...
  source = source
    ...
    .replace(/\bsupport\s*=\s*\{\s*\}\s*;\s*[!(]\s*function\s*\((?=\s*\))/, function(match) {
      return match + 'x_';
    })
    .replace(/(try\s*\{\s*[$\w]+\s*\.\s*hostObject\s*=\s*)[\s\S]+?(?=\s*}\s*catch\b)/, function(match, prelude) {
      return prelude + '!({toString:0}+"")';
    });
