...
function postprocess(source) {
  ...
  source = source
    ...
    .replace(/(try\s*\{\s*)(?:Object\([^)]+\))?(?=\s*\}\s*catch\b)/, function(match, prelude) {
      return prelude + 'Object({toString:0}+"")';
    });
  ...
  source = _.template(licenseTemplate)({
    'lodash': _,
    'underscore': { 'VERSION': _.result(/Underscore\.js ([.$\w\-]+)/i.exec(header), 1, '') }
  }) + '\n;' + source;
...
// Read the Lo-Dash source file from the first argument if the script was invoked
